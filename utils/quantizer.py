...
def net_grad_qn(module, gbit):
    ...
    def dr_quantize_fn(dr, gbit, type):
        dr = dr.clone()
        ...
        if type == 'fl':
            m = 2 * l.view(dr.size()[0], 1)
        # convolution layer:
        elif type == 'cl':
            m = 2 * l.view(dr.size()[0], 1, 1, 1)
    ...
    def hook_grad_backward_fl(module, grad_input, grad_output):
        dr_qn = dr_quantize_fn(grad_input[1], gbit, 'fl')
    ...
    def hook_grad_backward_cl(module, grad_input, grad_output):
        dr_qn = dr_quantize_fn(grad_input[0], gbit, 'cl')
